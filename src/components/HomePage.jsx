  ...
  return (
    <>
        ...
        <section className="one">
          <div className="pix">
            ...
            <div key={index} className={index === currentIndex ? " pix" : ""} >
                {index === currentIndex && <img key={index} src={item.desktopImage} alt="hero-1" className="desktop-hero-1" />} 
                {index === currentIndex && <img key={item.id} src={item.mobileImage} alt="hero-1" className="hero-1" />}
            </div>
            ))}
          </div>
          ...
          <nav className="nav">
            <div className="menu">
              {!open && <img src="/icon-hamburger.svg" alt="icon" className="hamburger" onClick={handleOpen} />}
              {open && <FaTimes className="close" onClick={handleOpen} />}
            </div>  
            <div className="desktop-logo">
              <h3>room</h3>
            </div>
            {!open && <div className="mobile-logo">
              <h3>room</h3>
            </div>}
          </nav>
        ...
        <section className="two">
          ...
          {storeItems.map((item, index) => (
            <div key={index} className={index === currentIndex ? "wrapper" : ""} >
                {index === currentIndex && <div className="discover"><h1>{item.title}</h1> </div> } 
                {index === currentIndex && <div className="description"><p>{item.description}</p></div>}
            </div>
            ))}
            <div className="shop">
              <h2>SHOP NOW</h2>
